package com.jnetaol.sshcommander.logger
...
object DebugLogger {
    ...
    fun init(context: Context): DebugLogger {
        ...
        initialized = true
        try {
            val logDir = File(context.filesDir, "sc_logs")
            if (!logDir.exists()) logDir.mkdirs()
            logFile = File(logDir, "sc_debug_${System.currentTimeMillis()}.log")
            writer = FileWriter(logFile, true)
            scope.launch { for (entry in writeChannel) writeToFile(entry) }
            d("DebugLogger", "Logger initialized", "SC-001", mapOf(
    ...
    fun d(tag: String, message: String, errorCode: String = "SC-000", metadata: Map<String, String> = emptyMap()) =
        log(LogLevel.DEBUG, tag, message, errorCode, null, metadata)
    fun i(tag: String, message: String, errorCode: String = "SC-000", metadata: Map<String, String> = emptyMap()) =
        log(LogLevel.INFO, tag, message, errorCode, null, metadata)
    fun w(tag: String, message: String, errorCode: String = "SC-000", metadata: Map<String, String> = emptyMap()) =
        log(LogLevel.WARN, tag, message, errorCode, null, metadata)
    fun e(tag: String, message: String, errorCode: String = "SC-ERR", throwable: Throwable? = null, metadata: Map<String, String> = emptyMap()) =
        log(LogLevel.ERROR, tag, message, errorCode, throwable, metadata)
    fun fatal(tag: String, message: String, errorCode: String = "SC-FATAL", throwable: Throwable? = null, metadata: Map<String, String> = emptyMap()) =
    ...
    fun exportLogsZip(): File? {
        return try {
            val exportFile = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "sc_debug_${System.currentTimeMillis()}.zip")
        ...
        } catch (e: Exception) { e("DebugLogger", "Export failed", "SC-ERR-001", e); writer = logFile?.let { FileWriter(it, true) }; null }
    }
